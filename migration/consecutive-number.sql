...
-- 2) Trigger: numero_acta = (máximo existente vs. piso configurado) + 1. Ignora lo que mande el form.
...
as $$
  select greatest(
           coalesce((select max(case when numero_acta ~ '^[0-9]+$' then numero_acta::bigint end)
                       from public.rc_tramites where tipo = p_tipo and ano = p_ano), 0),
...
insert into public.rc_numero_inicio (tipo, ano, desde) values
  ...
  ('naturalizacion', 2026, 6)     -- naturalización / nacionalidad
